...
err_t qio_readv(fd_t fd, qbuffer_t* buf, qbuffer_iter_t start, qbuffer_iter_t end, ssize_t* num_read)
{
  ...
  if( num_items < 0 || num_parts < 0 || num_parts > INT_MAX ) {
    return EINVAL;
  }
...
static
err_t _qio_channel_makebuffer_unlocked(qio_channel_t* ch)
{
  err_t err;
  ...
  if( (ch->hints & QIO_METHODMASK) == QIO_METHOD_MMAP &&
    ...
    if( ch->file->mmap ) {
      ...
      if( uselen > 0 ) {
        ...
        err = qbuffer_append(&ch->buf, ch->file->mmap, start, uselen, 0);
      }
...
static inline
err_t _qio_channel_needbuffer_unlocked(qio_channel_t* ch)
{
...
err_t _qio_channel_final_flush_unlocked(qio_channel_t* ch)
{
      ...
      if( !err ) {
        ...
        if( method == QIO_METHOD_MMAP ) {
          ...
          if( cur_end > max_written ) {
            qbuffer_shrink_back( ch->file->buf, max_written );
            // was qbuffer_trim_back( ch->file->buf, cur_end - max_written );
          }
        }
  ...
  if( qbuffer_is_initialized(&ch->buf) ) {
    // Destroy the buffer.
    err = qbuffer_destroy_inplace(&ch->buf);
  }
...
err_t _buffered_allocate_bufferspace(qio_channel_t* ch, int64_t amt, int64_t max_amt)
{
  ...
  while( left > 0 ) {
    ...
    err = qbuffer_append(&ch->buf, tmp, 0, uselen, 0);
...
err_t _buffered_get_memory_file_lock_held(qio_channel_t* ch, int64_t amt, int writing)
{
  ...
  if( writing ) {
    ...
    while( end > qbuffer_end_offset(ch->file->buf) ) {
      ...
      if( err ) goto error;
      err = qbuffer_append(ch->file->buf, iobuf, 0, qbytes_len(iobuf), 0);
      qbytes_release(iobuf);
      if( err ) goto error;
    }*/
    err = qbuffer_grow_back(ch->file->buf,
  ...
  if( qbuffer_iter_num_items(fstart, fend) < amt ) eof = 1;
  ...
  err = qbuffer_append_buffer(&ch->buf, ch->file->buf, fstart, fend, 0);
  if( err ) goto error;

  // Update av_end to be pointing to the end of the buffer.
  ch->av_end += qbuffer_iter_num_items(fstart, fend);
...
err_t _buffered_get_mmap(qio_channel_t* ch, int64_t amt_in, int writing)
{
  ...
  if( len > 0 ) {
    ...
    err = qbuffer_append(&ch->buf, bytes, skip, len - skip, 0);
...
void _qio_buffered_setup_cached(qio_channel_t* ch)
{
  ...
  if( av_bytes > 0 ) {
    ...
    qbuffer_iter_get_items(&ch->buf, start, end, &bytes, &skip, &len);
...
err_t _qio_buffered_behind(qio_channel_t* ch, int flushall)
{
  ...
  if( qbuffer_iter_num_items(write_start, write_end) == 0 ) {
    err = 0;
    goto done;
  }
  ...
  if(ch->flags & QIO_FDFLAG_WRITEABLE) {
    while( qbuffer_iter_num_items(write_start, write_end) > 0 ) {
      err = EINVAL;
      num_written = 0;
      switch (method) {
        case QIO_METHOD_READWRITE:
        ...
        case QIO_METHOD_MEMORY:
          ...
          err = 0;
          num_written = qbuffer_iter_num_items(write_start, write_end);
          break;
        // no default to get warnings when new methods are added
      }
      if( err ) goto error;
...
error:
  ...
  qbuffer_trim_front(&ch->buf, qbuffer_iter_num_items(start, write_start));
...
err_t _qio_channel_require_unlocked(qio_channel_t* ch, int64_t amt, int writing)
{
  err_t err;
...
err_t _qio_buffered_write(qio_channel_t* ch, const void* ptr, ssize_t len, ssize_t *amt_written)
{
  ...
  gotlen = qbuffer_iter_num_items_after(&ch->buf, end);
  ...
  if( method == QIO_METHOD_MMAP &&
    ...
    while( len > 0 ) {
      ...
      switch (method) {
        ...
        case QIO_METHOD_PREADPWRITE:
          err = sys_pwrite(ch->file->fd, ptr, len, _right_mark_start(ch), &num_written);
          break;
        case QIO_METHOD_FREADFWRITE:
          if( ch->file->fp ) {
  ...
  if( ch->file->mmap && _right_mark_start(ch) + len <= ch->file->mmap->len) {
    ...
    while( len > 0 ) {
      ...
      switch (method) {
        ...
        case QIO_METHOD_MMAP:
        case QIO_METHOD_PREADPWRITE:
          err = sys_pread(ch->file->fd, ptr, len, _right_mark_start(ch), &num_read);
          break;
        case QIO_METHOD_FREADFWRITE:
          if( ch->file->fp ) {
...
err_t _qio_channel_put_bytes_unlocked(qio_channel_t* ch, qbytes_t* bytes, int64_t skip_bytes, int64_t len_bytes)
{
  ...
  if( use_len > 0 ) {
    ...
    if( method == QIO_METHOD_MEMORY ) {
      err = qbuffer_append( ch->file->buf, bytes, use_skip, use_len, 0);
      if( err ) goto error;
    }

    // Append to the channel.
    err = qbuffer_append(& ch->buf, bytes, use_skip, use_len, 0);
    if( err ) goto error;
...
err_t _qio_channel_put_buffer_unlocked(qio_channel_t* ch, qbuffer_t* src, qbuffer_iter_t src_start, qbuffer_iter_t src_end)
{
  ...
  // check that we don't write past the channel's region.
  {
    ...
    start = _right_mark_start(ch);
    end = start + qbuffer_iter_num_items(src_start, src_end);
    if( end > ch->end_pos ) {
      end = ch->end_pos;
      src_end = src_start;
      qbuffer_iter_advance(src, &src_end, end - start);
    }
    use_len = qbuffer_iter_num_items(src_start, src_end);
  }
  ...
  if( use_len > 0 ) {
    ...
    // We take care of locking file above
    if( method == QIO_METHOD_MEMORY ) {
      err = qbuffer_append_buffer( ch->file->buf, src, src_start, src_end, 0);
      if( err ) goto error;
    }

    // Append to the channel.
    err = qbuffer_append_buffer(& ch->buf, src, src_start, src_end, 0);
    if( err ) goto error;
...
err_t qio_channel_mark_maybe_flush_bits(const int threadsafe, qio_channel_t* ch, int flushbits)
{
  ...
  if( ch->mark_cur + 1 >= ch->mark_stack_size ) {
    ...
    if( ch->mark_stack == ch->mark_space ) {
      ...
      new_buf = qio_realloc(ch->mark_space, new_size*sizeof(int64_t));
      if( ! new_buf ) {
        err = ENOMEM;
        goto error;
      }
