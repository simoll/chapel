...
Alternatively, the implementation could support a `move` operation
...
`move` operation - does nothing (other than
memcpy). The `_refcnt` `postblit` - which is called in a `copy` operation
- bumps the reference count.  Any
...
depending on user input, but no `copy` operation is necessary.
...
because a `move` operation is invoked in the `return` from `f` and `isalias` is
...
* Arrays could return always by ref, or sometimes by ref and sometimes by
  ...
  If they always returned by ref, an array return `copy` operation could set
...
intent. We might consider enabling the `out` intent to be used as an
explicit return intent that is the alternative to `ref`.  If we did so,
...
an array, the version using `=` would perform a deep copy of the array
elements.
