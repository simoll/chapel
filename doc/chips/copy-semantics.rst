 ...
 * `move(type, src)` refers to a move operation. The move operation
   ...
   In other words, this AST snippet:

   ::

    prim_move dst, move(t, src)
...
type as a run-time argument to the `copy` and `move` operations.  This
...
In addition, if some copy operations continue to be added early in compilation
in the normalize pass, it is also useful to pass the static type to the
`copy` and `move` operations for types that do not have a runtime type.
...
what the post-normalization and post-resolution AST should look like.
As discussed in :ref:`where-handled`, we may choose not to add
these operations before resolution. The AST listings are here only to
aid in clearly communicating when the copy and move operations are added.
...
The rules cover typed and un-typed cases. The rules perform the same `move`
or `copy` operations whether typed or un-typed. When types are explicit, these
`move` or `copy` operations need any run-time component of the type in order
...
this example should perform the `move` operation, transferring the
responsibility of destroying the returned value to the variable.

It might be possible to omit this `move` operation entirely. This proposal keeps
this `move` operation for symmetry with the next case, where it is required for
...
It might be possible to omit this `move` operation entirely, but it is at least
...
This example should perform the `copy` operation. It needs to do so because
...
::
  ...
  prim_move a, copy(t, tmp)

(a type without a runtime type would simply omit the `rtt` argument).


Alternatively, the implementation could support a `copy` operation
...
This example should perform the `copy` operation for the same reasons as the
...
Assuming that `f` returns by value (and not with `ref` return intent),
no `copy` operation is necessary under the proposed semantics.
The process of returning transfers the responsibility for destroying
the result of `f` to the caller of `g`.

This case could be implemented without any `move` operation, but for
symmetry with the next case, where the `move` operation is required for
types with a runtime type, a `move` operation is included. Including a
`move` in this case also helps with array slices (see
:ref:`array-slices`).
...
For that reason, a `move` operation is necessary under the proposed semantics, at
...
In this case, no `copy` operation is necessary under the proposed
semantics. The responsibility for destroying `local` is transferred to
the caller of `g`.

This case could be implemented without any `move` operation, but it is
...
For that reason, a `move` operation is necessary under the proposed semantics,
...
argument by `ref` or `const ref`, no `move` or `copy` operation is necessary
at this time. If `f` uses `ref` or `const ref` return intent, and `g()`
takes in its argument with the `in` intent, a `copy` operation will be added
 ...
 * The `return` statement in a `ref` or `const ref` return intent
   function does not cause a `move` or `copy` operation to be added. The usual
 ...
 * Returning a local variable or temporary in a function with `ref` return
   ...
   and so should be a compile error if possible. In other words,
   it is a program error to return a ref to any variable that
   is allocated on the stack. It is a program error in a ref-return
   function to:
 ...
 * enabling some types to specify another type to serve as their `ref` type.
   In that case, the `move` or `copy` operation would be added as normal, but
...
compiler.  The section :ref:`automatic-in-arg-intent` describes a
strategy that could eliminate this unnecessary copy.
...
alternative is to use a code transformation to remove unnecessary copies
...
The `ref` return intent could be added automatically.
...
Note that for non-array types, it it is already illegal to assign to the
result of such a call. For example
...
Distinguishing between `move` operations
++++++++++++++++++++++++++++++++++++++++

In an argument to `move` operations, distinguish between:

 * `move` operations made as part of function return
 * `move` operations made as part of user variable initialization
...
It might also be useful for `move` and `copy` operations to have an
argument indicating whether or not the source value is a a dead/expiring
value.
...
replaced by using the `move` operation that takes in a type argument.
...
they might need adjustment:
  ...
  * scopeResolve adds initCopy calls in some cases when creating a type
    constructor or object constructor. These could just be `copy` operations.
  ...
  * functionResolution.cpp captureTaskIntentValues adds an autoCopy call
    ...
    perhaps be removed. If not, it could be a `copy` operation.
  ...
  * parallel.cpp insertAutoCopyDestroyForTaskArg adds an autoCopy for
    ...
    for `string` is a workaround that could perform the `copy` operation
