...
module ChapelTaskData {
  ...
  // 16 bytes of wide pointer for ErrorGroup
  // 1 byte of bool for serial_state
  private const chpl_offset_endCount = 0;
  private const   chpl_offset_error = 16;
  private const  chpl_offset_serial = 32;
  private const     chpl_offset_end = c_sizeof(bool);
  ...
  proc chpl_setErrorGroup(in group: ErrorGroup) {
    ...
    // Using memcpy to avoid pointer type punning
    c_memcpy(c_ptrTo(prv[i]), c_ptrTo(group), 16);
  ...
  proc chpl_getErrorGroup() : ErrorGroup {
    var ret:ErrorGroup = nil;
    var prv = chpl_task_getChapelData():c_ptr(c_uchar);
    var i = chpl_offset_error;
    // Using memcpy to avoid pointer type punning
    c_memcpy(c_ptrTo(ret), c_ptrTo(prv[i]), 16);
